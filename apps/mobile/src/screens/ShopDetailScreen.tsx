    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.infoCard}>
          ...
          <Text style={styles.meta}>{t('shop.monthlySales')}{shop.monthlySales} · {shop.address}</Text>
          <Text style={styles.meta}>{t('shop.deliveryFee')}¥{shop.deliveryFee} · ¥{shop.minOrder}{t('shop.minOrder')}</Text>
          ...
          {(['menu', 'reviews', 'info'] as const).map((tab) => (
            <TouchableOpacity key={tab} style={[styles.tab, activeTab === tab && styles.tabActive]} onPress={() => setActiveTab(tab)}>
              <Text style={[styles.tabText, activeTab === tab && styles.tabTextActive]}>
                {tab === 'menu' ? t('shop.menu') : tab === 'reviews' ? t('shop.reviews') : t('shop.shopInfo')}
              </Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        {activeTab === 'menu' && (
          <View style={styles.mapSection}>
            <View style={styles.mapHeader}>
              <Text style={styles.mapTitle}>{t('shop.shopLocation')}</Text>
              <TouchableOpacity
            ...
            {group.products.map((product: any) => (
              <TouchableOpacity key={product.id} style={styles.productRow} onPress={() => navigation.navigate('ProductDetail', { id: product.id })}>
                ...
                <View style={styles.productInfo}>
                  ...
                  <Text style={styles.productMeta}>{t('shop.monthlySales')}{product.sales}</Text>
                </View>
        ...
        {activeTab === 'reviews' && <EmptyView message={t('shop.noReviews')} />}
        {activeTab === 'info' && (
          <View style={styles.infoSection}>
            <Text style={styles.infoLabel}>{t('shop.shopName')}</Text>
            <Text style={styles.infoValue}>{shop.name}</Text>
            <Text style={styles.infoLabel}>{t('shop.address')}</Text>
            <Text style={styles.infoValue}>{shop.address}</Text>
            <Text style={styles.infoLabel}>{t('shop.businessHours')}</Text>
      ...
      <View style={styles.bottomBar}>
        ...
        <View style={styles.bottomInfo}>
          ...
          <Text style={styles.bottomMeta}>{t('shop.extraDeliveryFee')}¥{shop.deliveryFee}</Text>
        </View>
        <View style={styles.bottomBtn}>
          <Text style={styles.bottomBtnText}>¥{Number(shop.minOrder).toFixed(0)}{t('shop.minOrder')}</Text>
        </View>
      </View>
    </View>
  );
